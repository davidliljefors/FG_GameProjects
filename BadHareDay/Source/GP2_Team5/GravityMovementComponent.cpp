...
FVector UGravityMovementComponent::ComputeSlideVector(const FVector& Delta, const float Time, const FVector& Normal, const FHitResult& Hit) const
{
	{
...
void UGravityMovementComponent::PhysFalling(float deltaTime, int32 Iterations)
{
	...
	while (RemainingTime >= MIN_TICK_TIME && Iterations < MaxSimulationIterations)
	{
		...
		auto LocalUp = CharacterOwner->GetActorUpVector();
		auto MoveDir = Velocity.GetSafeNormal();

		// If We are moving mostly upwards, dont be too harsh with collision
		// We might be jumping against a wall to the right which we want to jump up onto and land

		if ((MoveDir | LocalUp) > 0.82f)
		{
			FHitResult TraceHit;
			FCollisionQueryParams CQParams;
			CQParams.AddIgnoredActor(CharacterOwner);
			auto HeightFromCenter = CharacterOwner->GetCapsuleComponent()->GetScaledCapsuleHalfHeight();
			GetWorld()->LineTraceSingleByChannel(TraceHit, CharacterOwner->GetActorLocation(),
				CharacterOwner->GetActorLocation() + HeightFromCenter * CharacterOwner->GetActorUpVector(), ECC_MAX, CQParams);

			// If something is above our head we can revert to normal collision

			if (!TraceHit.bBlockingHit)
			{
				SafeMoveUpdatedComponent(Adjusted, PawnRotation, false, Hit);
			}
			else
			{
